...
#pragma warning restore 649
        ...
        private float RotationXRaw()
        {
                ...
                return input.RotateX(platform,movement_joystick,touch_panal) * mouseSensitivity;
        ...
        private float RotationYRaw()
        {
                ...
                return input.RotateY(platform,movement_joystick,touch_panal) * mouseSensitivity;
        ...
        {
            var direction = new Vector3(input.Move(platform,movement_joystick,touch_panal), 0f, input.Strafe(platform,movement_joystick,touch_panal)).normalized;
            var worldDirection = transform.TransformDirection(direction);
            var velocity = worldDirection * (input.Run(platform,movement_joystick,touch_panal) ? runningSpeed : walkingSpeed);
        ...
        private void Jump()
        {
            if (!_isGrounded || !input.Jump(platform,movement_joystick,touch_panal)) return;
            _isGrounded = false;
        ...
        private void PlayFootstepSounds()
        {
            ...
            {
                _audioSource.clip = input.Run(platform,movement_joystick,touch_panal) ? runningSound : walkingSound;
        ...
        private class FpsInput
        {
            ...
            public float RotateX(string platform,FixedJoystick movement_joystick,FixedTouchField touch_panal )
            {

                if (platform == "pc") { 
             return Input.GetAxisRaw(rotateX);

                }
                else if (platform == "mobile")
                {
                    return touch_panal.TouchDist.x;
            ...
            public float RotateY(string platform,FixedJoystick movement_joystick,FixedTouchField touch_panal )
            {
                if (platform == "pc")
                {
                    return Input.GetAxisRaw(rotateY);

                }
                else if (platform == "mobile")
                {
                    return touch_panal.TouchDist.y;
            ...
            public float Move(string platform,FixedJoystick movement_joystick,FixedTouchField touch_panal )
            {
                if (platform == "pc")
                {
                    ...
                    return movement_joystick.Horizontal;
                }
                else
                {
                    return 0;
            ...
            public float Strafe(string platform,FixedJoystick movement_joystick,FixedTouchField touch_panal )
            {
                ...
                {
                    return movement_joystick.Vertical;
                }
                else
                {
                    return 0;
            ...
            public bool Run(string platform,FixedJoystick movement_joystick,FixedTouchField touch_panal )
            {
                return Input.GetButton(run);
            }
				     
            /// Returns true during the frame the user pressed down the virtual button mapped to jump.          
            public bool Jump(string platform,FixedJoystick movement_joystick,FixedTouchField touch_panal )
            {
